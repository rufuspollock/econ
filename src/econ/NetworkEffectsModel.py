# Important reference for a) notation b) algorithms, as well as general companion is
# [1] A Model of Porting with Special Attention to Microsoft v2, 2005, Rufus Pollock
# see http://www.thefactz.org/economics/
...
# plan
# 1. define nu(N) and h(t)
# 2. plot summary diagram
# 3. calculate zeroes. Define a function networkSize(constants)
# 4. solve for varying p
# 5. solve for varying f

class NetworkEffectsModel:
	"""
	Class both contains model information and provides helper functions to find e.g. equilibria.
	...
	[[TODO: throughout we have problems when multiple equilibria since either:
		a. don't know which one the numerical search will use (and this messes up other stuff that depends on it such as profit maximization
		b. have to provide bounds within which to search which give only one equilibria]]
	"""
	...
	def __init__(self, nu_A, nu_B, het_A, het_B, priceFunction_A, priceFunction_B, constantList = []):
		...
		Comments
		========
		1. It is required that one deal with arbitrary functional forms for network effect and heterogeneity.	This means that we also need to deal with arbitrary set of variables hence the constantList parameter
		2. There are 2 platforms A,B
		3. Measure of consumers if 1 and they are indexed by t. This index is assumed to be the first argument for heterogeneity functions.
		...
		@param: priceFunction_A: these are independent of index and dependent only on the platform chosen
	...
	def equilibriaFunction(self, tindex):
		...
		Interior equilibria are zeroes of utility_A = utility_B and therefore equilibria [1]
	...
	def getDemand(self, price, startSearch, endSearch):
		...
		Currently user specifies an area [startSearch, endSearch] to search in which there is one and only one equilibrium.
		Then find
			1. local equilibrium t1
			2. local maximum of g(t) at t2 (for stable equlibrium can assume t2 < t1) 
		So search in [t2, t1]
		
		[[TODO:? deal with unstable case or warn about it]]
		@param price: price of platform A
		@param startSearch: [startSearch, endSearch] is an area in which to search for equilibrium in which there is one and only one equilibrium.
		@param endSearch: [startSearch, endSearch] is an area in which to search for equilibrium in which there is one and only one equilibrium.
		"""
		...
		# if price too high no local solution for that demand so return -ve demand
	...
	def _findMaximalAllowablePrice(self, startSearch, endSearch):
		"""
		Find (locally) maximal price that can be charged before getting discontinuity in demand
	...
	def solveMonopolistPriceProblem(self, startSearch, endSearch):
		"""
		Get profits for monopolist on platform A by solving the profit maximization problem.
		
		@param startSearch: see @param{endSearch}
		@param endSearch: define interval [startSearch, endSearch] in which to search when inverting equilibrium function to find demand
	...
	def getConsumerWelfare(self, equilibriumValue):
		"""
		Calculate total consumer welfare W = \int_{A} u(t,A) dt + \int_{B} u(t,B) dt.
		Can break this up as:
		W = Network Effect Benefits + Heterogeneity Stuff
		W = t^{e} * \nu(t^{e}, A) + (1-t^{e}) * \nu(1-t^{e}) + Heterogeneity stuff
		...
		networkBPart = (1.0 - equilibriumValue) * self._nu_B(1.0 - equilibriumValue)
		...
		return networkAPart + networkBPart + hetAPart + hetBPart + priceAPart + priceBPart