# Models to analyze copyright
# (c) 2005 Rufus Pollock. See License for details

# Some Terminology (see corresponding paper for more details)
...
class CopyrightIncomeModel:
	...
	def getIncome(self, startDate, period = 1):
		...
		Assume income is adjusted so that it is as if a single lump sum were paid at endDate (so cumulate as necessary)
		"""
		pass
...
class CopyrightIncomeSummary:
	...
	Assume: that non-PV income is constant or declining eventually (in fact at least by 500 years out). This assumption is sufficient to assure theoretical and numerical convergence
	
	TODO: need to sort out threshold to be better
	"""
	
	# can't define these here but these variables will exist
	# discountRate = None
	# incomeModel = None
	
	# do we delay income model by one period?
	firstPaymentDelay = 0
	# max time period to calculate until in getting income (to stop overrunning memory etc)
	maxTimePeriod = 1000
	minTimePeriod = 150
	...
	def calculate(self):
		...
		Calculate summary information. Specifically:
			...
			Calculate I(t)
			Calculate total possible income (assumed to be what we have at end of iteration)
		
		results are stored in presentValueOfIncome
		keeps running until PV of income being added is negligible or we reach maxTimePeriod
		"""
		self.presentValueOfIncome = []
		self.cumulativeTotals = []
		total = 0
		incomePV = self.incomeThreshold + 1 # just define to anything > incomeThreshold
		ii = 0
		while incomePV > self.incomeThreshold or ii < self.minTimePeriod:
			...
			if ii > self.maxTimePeriod:
				# break out anyway
				logger.warning(
					"Income PV was NOT below threshold after " + str(self.maxTimePeriod) + \
	...
	def getProportions(self):
		"""
		@return a list where entry i = proportion of total possible income earned by period i (i.e. I(i) / I(infty)) 
		"""
...
if __name__ == "__main__":
	...
	# TODO:
	# really want to give people a gui where they they can enter things like discount rate. Different values for income model etc
