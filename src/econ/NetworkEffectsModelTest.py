...
class NetworkEffectsModelTest(unittest.TestCase):
	"""
	We test the network effects model by setting up a specific case we have	examined extensively by hand (see references in other files for details). We also use this file to set up various network effect models which may be used elsewhere.
	"""
	
	def __init__(self, methodName = 'testGetEquilibrium'):
		"""
		Need this so that we can instantiate the class outside of test framework for use in graphing etc
		"""
	...
	def linearNetworkEffects(self, aa, bb1, bb2):
		"""
		This implements the standard linear model (both network effects and heterogeneity
	...
	def locationalINESymmetric(self, aa, bb, alpha = 1.0):
		"""
		Symmetric indirect network effects (based on locational model of imperfect competition) model
